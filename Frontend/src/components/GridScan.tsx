...
export const GridScan = ({
  ...
  const pushScan = (t: number) => {
    ...
    if (materialRef.current) {
      ...
      (u.uScanStarts as any).value = buf;
      (u.uScanCount as any).value = arr.length;
    }
  };
  ...
  useEffect(() => {
    ...
    const onResize = () => {
      renderer.setSize(container.clientWidth, container.clientHeight);
      (material.uniforms.iResolution as any).value.set(container.clientWidth, container.clientHeight, renderer.getPixelRatio());
    ...
    const tick = () => {
      ...
      (material.uniforms.uSkew as any).value.set(skew.x, skew.y);
      (material.uniforms.uTilt as any).value = tiltCurrent.current * tiltScale;
      (material.uniforms.uYaw as any).value = THREE.MathUtils.clamp(yawCurrent.current * yawScale, -0.6, 0.6);
      (material.uniforms.iTime as any).value = now / 1000;
  ...
  useEffect(() => {
    ...
    if (m) {
      const u = m.uniforms;
      (u.uLineThickness as any).value = lineThickness;
      (u.uLinesColor as any).value.copy(srgbColor(linesColor));
      (u.uScanColor as any).value.copy(srgbColor(scanColor));
      (u.uGridScale as any).value = gridScale;
      (u.uLineStyle as any).value = lineStyle === 'dashed' ? 1 : lineStyle === 'dotted' ? 2 : 0;
      (u.uLineJitter as any).value = Math.max(0, Math.min(1, lineJitter || 0));
      (u.uBloomOpacity as any).value = Math.max(0, bloomIntensity);
      (u.uNoise as any).value = Math.max(0, noiseIntensity);
      (u.uScanGlow as any).value = scanGlow;
      (u.uScanOpacity as any).value = Math.max(0, Math.min(1, scanOpacity));
      (u.uScanDirection as any).value = scanDirection === 'backward' ? 1 : scanDirection === 'pingpong' ? 2 : 0;
      (u.uScanSoftness as any).value = scanSoftness;
      (u.uPhaseTaper as any).value = scanPhaseTaper;
      (u.uScanDuration as any).value = Math.max(0.05, scanDuration);
      (u.uScanDelay as any).value = Math.max(0.0, scanDelay);
    }
    if (bloomRef.current) {
